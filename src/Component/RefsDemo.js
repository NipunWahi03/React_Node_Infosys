    ...
    render() {
        return (
            <div>
                ...
                <input type="text" ref={this.textRef}></input>{/*  1st approach-attaching the new ref property to the element using the ref keyword -2nd step*/}
                <input type="text" ref={this.cbRefMethod}></input>{/*  2nd approach-attaching the new ref property to the element using the ref keyword -3rd step*/}
                <button onClick={this.getElementData}>Click to fetch the data from the Text field</button>
            </div>
        )
    }
}
